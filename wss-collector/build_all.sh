...
# 全平台构建脚本
# 编译Rust项目到多个目标平台
# 使用方法: ./build_all.sh [all]
# 默认只编译Linux x64，添加 'all' 参数编译所有平台
...
if [ "$BUILD_ALL" = "all" ]; then
    TARGETS=("${ALL_TARGETS[@]}")
    echo -e "${BLUE}模式: 编译所有平台${NC}"
else
    TARGETS=("${DEFAULT_TARGETS[@]}")
    echo -e "${BLUE}模式: 仅编译Linux x64（使用 './build_all.sh all' 编译所有平台）${NC}"
fi
...
echo -e "${GREEN}开始编译 $total_count 个目标平台...${NC}"
...
if [ "$BUILD_ALL" = "all" ]; then
    ...
    echo -e "${YELLOW}Linux x64:${NC}    ./$OUTPUT_DIR/linux-x64/$PROJECT_NAME"
    echo -e "${BLUE}提示：使用 './build_all.sh all' 编译所有平台${NC}"
fi
echo -e "${GREEN}===========================================${NC}"
