...
impl KVDB {
    ...
    pub fn create_backup<P: AsRef<Path>>(&self, backup_dir: P) -> Result<PathBuf, KVError> {
        ...
        let file =
            File::create(&backup_path).map_err(|err| KVError::BackupIo(backup_path.clone(), err))?;
    ...
    pub fn restore_from<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        fs::create_dir_all(target)
            .map_err(|err| KVError::CreateDir(target.to_path_buf(), err))?;

        let file = File::open(backup).map_err(|err| KVError::RestoreIo(backup.to_path_buf(), err))?;
